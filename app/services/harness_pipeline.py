...
This module makes the existing 知溯 knowledge workflow explicit and
...
from app.utils import normalize_name as _normalize_name
...
class HarnessPipeline:
    ...
    def run(
        ...
        # 关键阶段失败则提前中止，避免下游 KeyError 级联产出损坏产物
        if trace.status == "failed" or not metadata.get("bvid"):
            return self._abort_result(run_id, run_dir, traces, stage_outputs, "ingest")

        transcript, trace = self._run_stage(
            "transcript",
            lambda: self._stage_transcript(metadata, transcript_source),
            input_summary=f"bvid={metadata.get('bvid', bvid)}, transcript_source={transcript_source}",
            artifact_path=run_dir / "raw_transcript.json",
        )
        traces.append(trace)
        stage_outputs["transcript"] = transcript
        if trace.status == "failed" or not transcript.get("segments"):
            return self._abort_result(run_id, run_dir, traces, stage_outputs, "transcript")
    ...
    def _abort_result(
        ...
        """关键阶段失败时的早退结果（写出 trace，避免下游级联崩溃）。"""
        pipeline_trace = {
            "pipeline_version": PIPELINE_VERSION,
            "run_id": run_id,
            "generated_at": _utc_now(),
            "artifact_dir": _path_ref(run_dir, self.root),
            "aborted_at": failed_stage,
            "stage_order": STAGE_ORDER,
            "stages": [t.to_dict() for t in traces],
        }
        _write_json(run_dir / "pipeline_trace.json", pipeline_trace)
        logger.warning("Harness pipeline aborted at stage=%s (run_id=%s)", failed_stage, run_id)
        return {
            "run_id": run_id,
            "artifact_dir": _path_ref(run_dir, self.root),
            "aborted_at": failed_stage,
    ...
    def _stage_validate(
        ...
        _name_counts = Counter(normalized_names)
        duplicates = sorted({name for name, c in _name_counts.items() if c > 1})
...
def load_latest_summary(output_root: str | Path = "artifacts/harness") -> dict[str, Any] | None:
