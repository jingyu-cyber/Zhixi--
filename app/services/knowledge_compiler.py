...
2. 每个 claim 的 statement 必须是视频字幕中真实出现的论述，不得脱离视频内容编造
3. claim 必须关联到对应的 concept
4. prerequisites 是视频中明确提到需要先掌握的概念
5. confidence < 0.3 不要输出
6. 不要抽取过于宽泛的概念（如"学习""内容""视频""方法""引言""介绍""总结"等）
7. 概念数量控制在 2-6 个，论断数量控制在 2-10 个
8. 如果视频内容确实很少（如只是简单介绍），宁可少抽取也要保证每个概念/论断有视频依据
9. definition 必须是视频中对该概念的实际讲解内容，不能用泛泛的百科式定义代替"""
...
def _enforce_node_limits(
    ...
    if len(concept_map) < MIN_CONCEPTS:
        supplements = _supplement_from_title(video_title, all_segment_text)
        added = 0
        for sup in supplements:
            norm = _normalize_name(sup["name"])
            if norm not in concept_map:
                concept_map[norm] = {
                    "name": sup["name"],
                    "normalized_name": norm,
                    "definition": sup.get("definition", f"视频《{video_title}》中涉及的概念"),
                    "difficulty": sup.get("difficulty", 1),
                    "source_count": 0,
                    "claims": [],
                }
                added += 1
                if len(concept_map) >= MIN_CONCEPTS:
                    break
        logger.info(
            f"[数量控制] 概念不足(原{len(concept_map) - added})，从标题补充 {added} 个 → 共 {len(concept_map)}"
        )

    # --- 论断不足5条：从文本提取补充 ---
    total_claims = sum(len(cdata.get("claims", [])) for cdata in concept_map.values())
    if total_claims < MIN_CLAIMS and all_segment_text:
        # 尝试从文本中提取更多论断（简单的规则提取）
        import re
        sentences = re.split(r"[。！？\.\!\?\n]", all_segment_text)
        extra_claims = []
        for sent in sentences:
            ...
            for norm, cdata in concept_map.items():
                name = cdata["name"]
                if name and len(name) >= 2 and name in sent:
                    extra_claims.append({
                        "statement": sent[:200],
                        "type": "explanation",
                        "confidence": 0.4,
                    ...
                    break
            if len(extra_claims) + total_claims >= MIN_CLAIMS:
                break

        # 将补充论断分配给第一个概念
        if extra_claims and concept_map:
            first_concept = next(iter(concept_map.values()))
            first_concept.setdefault("claims", []).extend(extra_claims[:MIN_CLAIMS - total_claims])
        logger.info(f"[数量控制] 论断不足，补充 {len(extra_claims)} 条")
